...
public class Lunar
{
	// 第几个月第几个星期日的节日（母亲节、父亲节）
	private String weekFestival = "";

	private void processWeekFestival()
	{
		weekFestival = "";
		...
		calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH),
				1);
		// 如果flag为1，表示WEEK_Of_MONTH为2，否则为2即可（母亲节）、父亲节，3或4，
		int increment = 1;
		...
		calendar.setTime(date);
		if (calendar.get(Calendar.DAY_OF_WEEK) == 1
				&& calendar.get(Calendar.WEEK_OF_MONTH) == (increment + 2)
				&& calendar.get(Calendar.MONTH) == 4)
			weekFestival = "母亲节";
		if (calendar.get(Calendar.DAY_OF_WEEK) == 1
				&& calendar.get(Calendar.WEEK_OF_MONTH) == (increment + 3)
	...
	{ 0x4bd8, 0x4ae0, 0xa570, 0x54d5, 0xd260, 0xd950, 0x5554, 0x56af, 0x9ad0,
			0x55d2, 0x4ae0, 0xa5b6, 0xa4d0, 0xd250, 0xd295, 0xb54f, 0xd6a0,
			0xada2, 0x95b0, 0x4977, 0x497f, 0xa4b0, 0xb4b5, 0x6a50, 0x6d40,
			0xab54, 0x2b6f, 0x9570, 0x52f2, 0x4970, 0x6566, 0xd4a0, 0xea50,
			0x6a95, 0x5adf, 0x2b60, 0x86e3, 0x92ef, 0xc8d7, 0xc95f, 0xd4a0,
			0xd8a6, 0xb55f, 0x56a0, 0xa5b4, 0x25df, 0x92d0, 0xd2b2, 0xa950,
			0xb557, 0x6ca0, 0xb550, 0x5355, 0x4daf, 0xa5b0, 0x4573, 0x52bf,
			0xa9a8, 0xe950, 0x6aa0, 0xaea6, 0xab50, 0x4b60, 0xaae4, 0xa570,
			0x5260, 0xf263, 0xd950, 0x5b57, 0x56a0, 0x96d0, 0x4dd5, 0x4ad0,
			0xa4d0, 0xd4d4, 0xd250, 0xd558, 0xb540, 0xb6a0, 0x95a6, 0x95bf,
			...
			0xad50, 0x55d9, 0x4ba0, 0xa5b0, 0x5176, 0x52bf, 0xa930, 0x7954,
			0x6aa0, 0xad50, 0x5b52, 0x4b60, 0xa6e6, 0xa4e0, 0xd260, 0xea65,
			0xd530, 0x5aa0, 0x76a3, 0x96d0, 0x4afb, 0x4ad0, 0xa4d0, 0xd0b6,
			0xd25f, 0xd520, 0xdd45, 0xb5a0, 0x56d0, 0x55b2, 0x49b0, 0xa577,
			0xa4b0, 0xaa50, 0xb255, 0x6d2f, 0xada0, 0x4b63, 0x937f, 0x49f8,
			0x4970, 0x64b0, 0x68a6, 0xea5f, 0x6b20, 0xa6c4, 0xaaef, 0x92e0,
			...
			0x4b55, 0x4b6f, 0xa570, 0x54e4, 0xd260, 0xe968, 0xd520, 0xdaa0,
	...
	{ "0101*元旦","0214 情人节", "0308 妇女节", "0312 植树节", "0315 权益日", "0401 愚人节",
			"0501*劳动节", "0504 青年节", "0601 儿童节","0701 建党节", "0801 建军节", 
			"0910 教师节",  "1001*国庆节", "1006 老人节","1111 光棍节", "1225 圣诞节" };
	/**
	 * 农历节日 *表示放假日
	 */
	private final static String[] lFtv =
	{ "0101*春节", "0115 元宵节","0505 端午节","0707 七夕节","0815 中秋节",
	...
	public static int getLunarMonthDays(int lunarYear, int lunarMonth)
	{
		// 数据表中,每个农历年用16bit来表示,
		// 前12bit分别表示12个月份的大小月,最后4bit表示闰月
		int daysInLunarMonth = ((Lunar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) == 0) ? 29
				: 30;
	...
	private void getCyclicalData()
	{
		...
		int firstNode = Lunar.getSolarTermDay(solarYear, solarMonth * 2); //传回当月「
		// 节
		// 」
		// 为几日开始
		// 依节气月柱, 以「节」为界
	...
	{
		if ("".equals(weekFestival))
			return this.sFestivalName;
		else if ("".equals(sFestivalName))
			return weekFestival;
		else
			return weekFestival + "/" + this.sFestivalName;
	}
